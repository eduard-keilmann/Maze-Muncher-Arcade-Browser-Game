...
export default {
  async fetch(request, env) {
    ...
    if (request.method === "POST") {
      ...
      if (body?.action === "submit") {
        ...
        try {
          ...
          const run = await env.LEADERBOARD_DB.prepare(
            "SELECT started_at, mode FROM leaderboard_runs WHERE id = ? AND expires_at > ?"
          ...
          const elapsedSeconds = (now - Number(run.started_at)) / 1000;
          if (
            !GAMEPLAY_MODES.has(run.mode) ||
            !Number.isFinite(elapsedSeconds) ||
            elapsedSeconds < 0 ||
            elapsedSeconds > RUN_TTL_MS / 1000 ||
            score > 10_000 + Math.floor(elapsedSeconds * 500)
          ) {
          ...
          await env.LEADERBOARD_DB.batch([
            ...
            ).bind(entryId, run.mode, name, score, createdAt),
            env.LEADERBOARD_DB.prepare(
          ...
          ).bind(run.mode, score, score, createdAt, createdAt, entryId).first();
      ...
      if (body?.action !== "start" || !GAMEPLAY_MODES.has(body.mode)) {
        return json(request, 400, { error: "Unknown leaderboard action or gameplay mode" });
      }
      ...
      try {
        ...
        await env.LEADERBOARD_DB.batch([
          ...
          env.LEADERBOARD_DB.prepare(
            "INSERT INTO leaderboard_runs (id, mode, started_at, expires_at) VALUES (?, ?, ?, ?)"
          ).bind(runId, body.mode, now, now + RUN_TTL_MS)
        ]);

        return json(request, 201, { runId, mode: body.mode });
      } catch (_) {
        return json(request, 503, { error: "Leaderboard unavailable" });
      }
    }
    ...
    if (!GAMEPLAY_MODES.has(mode)) {
      return json(request, 400, { error: "Unknown gameplay mode" });
    }

    try {
      const entries = await env.LEADERBOARD_DB.prepare(
        "SELECT id, name, score, created_at FROM leaderboard_entries WHERE mode = ? ORDER BY score DESC, created_at ASC, id ASC LIMIT ?"
      ).bind(mode, 100).all();
