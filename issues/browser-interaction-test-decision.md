...
- Pause and long-press restart actions exist.
- Short portrait layout contract exists.
- Held D-pad active state exists.
- Landscape layout contract exists.
- Whole-page swipe contract exists.
- Continuous swipe steering before finger release exists.
- Canvas tap-to-turn contract exists.
- Sound/music Web Audio controls exist.
- QA and decision artifacts exist.

Manual Safari/Chrome QA checklist remains required because real mobile browser behavior can still differ from static source checks, especially for touch event timing, viewport chrome, scroll behavior, audio unlock, and long-press feel.
...
- Whole-page continuous swipe is delayed, blocked, or causes page panning.
- Canvas tap-to-turn fights normal continuous swipe.
- Long-press restart misfires or fails on mobile browsers.
- Sound/music unlock differs across mobile browsers.
...
- one whole-page continuous swipe outside the canvas;
- one canvas tap-to-turn;
- pause at a mobile viewport;
- long-press restart at a mobile viewport;
- sound/music toggle after a user gesture;
